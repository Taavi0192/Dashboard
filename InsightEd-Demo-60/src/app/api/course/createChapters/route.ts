...
export async function POST(req: Request, res: Response) {
  try {
    ...
    let output_units: outputUnits = await strict_output_v2(
    ...
    const imageSearchTerm = await strict_output_v2(
...
// // 1st was fine till sep 2 ,then this
...
// export async function POST(req: Request) {
//   try {
//     // Get the session and check user authentication
//     const session = await getAuthSession();
//     if (!session?.user) {
//       return new NextResponse("Unauthorized", { status: 401 });
//     }

//     // Check user's credits or subscription status
//     const isPro = await checkSubscription();
//     if (session.user.credits <= 0 && !isPro) {
//       return new NextResponse("No credits", { status: 402 });
//     }
