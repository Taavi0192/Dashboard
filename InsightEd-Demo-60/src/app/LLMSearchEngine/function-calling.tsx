...
export async function goShopping(message: string) {
    ...
    try {
        const response = await fetch(url, requestOptions);
        if (!response.ok) {
            throw new Error(`Failed to fetch shopping data. Status: ${response.status}`);
        }
        const responseData = await response.json();
        const shoppingData = {
            type: 'shopping',
...
export async function getTickers(ticker: string) {
    try {
        ...
        return JSON.stringify({ error: 'Failed to get ticker data' });
    }
}
...
export async function searchSong(query: string): Promise<string> {
    try {
        ...
        return JSON.stringify({ error: 'Failed to search for song' });
    }
}
