...
const OpenEnded = ({ game }: Props) => {
  ...
  const { mutate: checkAnswer, isLoading: isChecking } = useMutation({
    mutationFn: async () => {
      ...
      //   input.value = "";
      // });
      document.querySelectorAll("#user-blank-input").forEach((inputElement) => {
        const input = inputElement as HTMLInputElement; // Asserting the type to HTMLInputElement
        filledAnswer = filledAnswer.replace("_____", input.value);
        input.value = ""; // Now TypeScript knows input has a value property
      });
